...
def digestParameters(Map properties=[:]) {
    ...
    // Yaml yaml = new Yaml()
    // yaml.load(defaultMap)
    // defaultMap =  evaluate(defaultMap)
    Map defaultMapYaml = new YamlSlurper().parseText(defaultMap)
...
def scan(Map properties=[:]) {
  ...
  withCredentials([string(credentialsId: "sonarqube-sa", variable: 'TOKEN')]) {
