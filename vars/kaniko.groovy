def push(Map properties=[:]) {
    ...
    def baseCommand="/kaniko/executor -f ${properties.build.workDir}/${properties.build.dockerFile} -c ${properties.build.workDir} -d ${tagList} ${ignorePathArg} --image-name-tag-with-digest-file=${env.GIT_COMMIT}-file-details --digest-file=${env.GIT_COMMIT}-image-properties --verbosity=debug --cache=${properites.build.cacheFlag} --cache-repo=${properties.build.artifactHost}/${properties.artifactPackagePath}-cache:${tag}"
