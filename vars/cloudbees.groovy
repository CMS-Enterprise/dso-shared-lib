...
def jenkinsUpdateToken(String refreshedToken) {
    ...
    withCredentials([string(credentialsId: 'jenkins-svc-api-token', variable: 'TOKEN')]) {
        sh """
            java -jar jenkins-jars/jenkins-cli.jar -s http://cjoc.cjoc/cjoc -http -auth dso-jenkins-dev-a:${TOKEN} update-credentials-by-xml system::system::jenkins _ JfrogArt-SA-ro-Token < update-string-cred.xml
